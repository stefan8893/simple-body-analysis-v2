...
import {
  BodyAnalysis,
  BodyAnalysisProperty,
} from '../body-analysis-data/body-analysis.types';
...
function calculateAverageWeeklyLossGain(
  ...
  const mondays = bodyAnalysisData.filter((x) => isMonday(x.analysedAt));

  if (mondays.length < 2) return null;

  const diff = mondays
    .slice(1)
    .map((x, i) => mondays[i][property] - x[property]);

  const agg = diff.reduce((acc, next) => acc + next, 0);

  return Math.abs(agg / diff.length);
}
